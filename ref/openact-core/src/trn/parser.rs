...
impl OpenActTrn {
    ...
    ///     ↓
    /// openact: trn:openact:tenant1:connection/github-{user_id}
    pub fn to_openact_trn(&self, user_id: &str) -> Result<String> {
        if !self.is_connection() {
            return Err(OpenActError::trn("Only connection TRNs can be converted to openact TRNs"));
        }

        Ok(format!(
            "trn:openact:{}:connection/{}-{}",
...
mod tests {
    ...
    #[test]
    fn test_to_openact_trn() {
        let trn = OpenActTrn::new("tenant1", "connection", "github", "v1");
        let openact_trn = trn.to_openact_trn("user123").unwrap();
        assert_eq!(openact_trn, "trn:openact:tenant1:connection/github-user123");
    }
