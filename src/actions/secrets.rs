...
impl SecretsProvider for MemorySecretsProvider {
    fn resolve(&self, uri: &str) -> Result<String> {
        self.items
            .get(uri)
            .cloned()
            .ok_or_else(|| anyhow!("secret not found for uri: {}", uri))
    }
}
...
impl SecretsProvider for VaultSecretsProvider {
    fn resolve(&self, uri: &str) -> Result<String> {
        ...
        let path = base
            .strip_prefix("vault://")
            .ok_or_else(|| anyhow!("invalid vault uri"))?;
        // Simplification: assume KV v2, mount is the first segment
        let mut parts = path.splitn(2, '/');
        let mount = parts
            .next()
            .ok_or_else(|| anyhow!("invalid vault uri mount"))?;
        let rest = parts
            .next()
            .ok_or_else(|| anyhow!("invalid vault uri path"))?;
        // Fetch secret data
...
impl<P: SecretsProvider> TaskHandler for SecretsResolveManyHandler<P> {
    fn execute(&self, _resource: &str, _state_name: &str, ctx: &Value) -> Result<Value> {
        let items = ctx
            ...
            .context("items required as object")?;
        let mut out = serde_json::Map::new();
        for (k, v) in items.iter() {
            let uri = v
                .as_str()
                .ok_or_else(|| anyhow!("invalid uri for key {}", k))?;
