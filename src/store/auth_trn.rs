//! openact TRN Identifier Definitions
//!
//! Defines standardized TRN format for the openact authentication system
...
impl AuthConnectionTrn {
    ...
    pub fn parse(trn_str: &str) -> Result<Self> {
        ...
        // Validate tool type
        if trn.tool() != "openact" {
            return Err(anyhow!("Expected tool 'openact', got '{}'", trn.tool()));
        }
...
impl AuthSessionTrn {
    ...
    pub fn parse(trn_str: &str) -> Result<Self> {
        ...
        if parts[0] != "trn" || parts[1] != "openact" || parts[3] != "session" {
            return Err(anyhow!("Invalid session TRN format"));
        }
...
impl AuthExecutionTrn {
    ...
    pub fn parse(trn_str: &str) -> Result<Self> {
        ...
        if parts[0] != "trn" || parts[1] != "openact" || parts[3] != "execution" {
            return Err(anyhow!("Invalid execution TRN format"));
        }
...
mod tests {
    ...
    fn test_auth_connection_trn() {
        ...
        assert_eq!(trn_str, "trn:openact:company123:connection/github-user456");
    ...
    fn test_auth_session_trn() {
        ...
        assert_eq!(
            trn_str,
            "trn:openact:company123:session:github-user456:sess_abc123def456"
        );
    ...
    fn test_auth_execution_trn() {
        ...
        assert_eq!(
            trn_str,
            "trn:openact:company123:execution:github-oauth2:exec_xyz789abc123"
        );
