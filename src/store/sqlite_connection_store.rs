...
use crate::{
    ...
    store::{
        ConnectionStore, AuthConnectionTrn,
...
impl SqliteConnectionStore {
    ...
    /// Record audit log
    async fn log_audit(&self, trn: &str, operation: &str, old_data: Option<&AuthConnection>, new_data: Option<&AuthConnection>, reason: Option<&str>) -> Result<()> {
        if !self.config.enable_audit_log {
            return Ok(());
        }
    ...
    pub async fn list_by_provider(&self, tenant: &str, provider: &str) -> Result<Vec<AuthConnection>> {
...
mod tests {
    ...
    async fn test_sqlite_connection_store() {
        ...
        let connection = AuthConnection::new("test_tenant", "github", "user123", "access_token_123").unwrap();
