...
#[cfg(feature = "server")]
use crate::authflow::engine::{RunOutcome, run_until_pause_or_end};
...
pub async fn execute_workflow(state: ServerState, execution_id: String) {
    let (workflow, flow_name, input, context) = {
        ...
        let execution = match executions.get(&execution_id) { Some(e) => e, None => return };
        let workflows = state.workflows.read().unwrap();
        let workflow = match workflows.get(&execution.workflow_id) { Some(w) => w.clone(), None => return };
    ...
    if let Some(ctx) = context {
        if let serde_json::Value::Object(ctx_map) = ctx { for (k, v) in ctx_map { exec_context.insert(k, v); } }
        else { exec_context.insert("context".to_string(), ctx); }
    }
    ...
    let start_state = {
        ...
        } else { flow.start_at.clone() }
    };

    let result = run_until_pause_or_end(flow, &start_state, exec_context, state.task_handler.as_ref(), 100);
