...
use stepflow_dsl::{WorkflowDSL, State, MappingFacade};
        ...
        match cmd {
            Command::ExecuteTask {
                ...
                if let Some(v) = context.get("vars").and_then(|v| v.as_object()) {
                    for (k, val) in v.iter() { vars.insert(k.clone(), val.clone()); }
                }
                if let Some(conn) = context.get("connection") { vars.insert("connection".into(), conn.clone()); }
                if let Some(secrets) = context.get("secrets") { vars.insert("secrets".into(), secrets.clone()); }
                ...
                let state_def = dsl.states.get(&state_name).ok_or_else(|| anyhow!("state not found: {}", state_name))?;
                ...
                if assign.is_some() {
                    ...
                    if let Some(assigned) = mapper.evaluate_assign(&assign, &ctx_assign)? {
                        for (k, v) in assigned { vars.insert(k, v); }
                    }
                }
                ...
                if output_mapping.is_some() {
                    ...
                    let _ = mapper.evaluate_output(&output_mapping, &raw_out, &state_name, &ctx_out, &mut dest)?;
        ...
        impl TaskHandler for Router {
            fn execute(&self, resource: &str, state_name: &str, ctx: &Value) -> Result<Value> {
                match resource {
                    "secrets.resolve" => {
                        let provider = crate::actions::MemorySecretsProvider::from_pairs(vec![
                            ("vault://kv/json", self.secret.as_str()),
                        ]);
                        crate::actions::SecretsResolveHandler::new(provider)
                            .execute(resource, state_name, ctx)
                    }
...
"#,
    ...
    fn connection_read_inject_and_refresh_update_flow() {
        ...
        let mut conn1 = crate::store::Connection::new("tenant1", "prov", "user1", "old").unwrap();
...
"#,
    ...
    fn ensure_auto_refresh_before_inject() {
        ...
        let store = MemoryConnectionStore::default();
        // expired token
        let mut conn2 = crate::store::Connection::new("tenant2", "prov", "user2", "old").unwrap();
