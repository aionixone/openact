...
impl ServerState {
    ...
    pub async fn from_env() -> Self {
        let (ws_broadcaster, _) = broadcast::channel(1000);
        // AUTHFLOW_STORE: memory | sqlite
        let store_env = std::env::var("AUTHFLOW_STORE").unwrap_or_else(|_| "memory".to_string());
        println!("[server] AUTHFLOW_STORE environment variable: {}", store_env);
        ...
        if store_env.eq_ignore_ascii_case("sqlite") {
            ...
            println!("[server] Using Memory backend (sqlite feature enabled but AUTHFLOW_STORE != 'sqlite')");
        }
...
async fn create_workflow(
    ...
    let parsed: AuthFlowDSL = match serde_json::from_value(normalized_json) {
        Ok(v) => v,
        Err(e) => {
            return (
                StatusCode::BAD_REQUEST,
