...
impl ExtensionHandler for ActionTypeHandler {
    fn handle(&self, key: &str, value: &Value) -> Result<ProcessedExtension> {
        ...
        let _action_type = value.as_str().ok_or_else(|| {
            OpenApiToolError::ValidationError("x-action-type must be a string".to_string())
        })?;
        ...
        Ok(ProcessedExtension {
            ...
            metadata: ExtensionMetadata {
                ...
                required: false,
                description: Some(
                    "Specifies the type of action (read, write, create, update, delete)"
                        .to_string(),
                ),
                validation_rules: vec![ValidationRule::Enum(vec![
    ...
    fn validate(&self, value: &Value) -> Result<()> {
        if let Some(action_type) = value.as_str() {
            match action_type {
                ...
                _ => Err(OpenApiToolError::ValidationError(format!(
                    "Invalid action type: {}. Must be one of: read, write, create, update, delete",
                    action_type
                ))),
            }
        } else {
