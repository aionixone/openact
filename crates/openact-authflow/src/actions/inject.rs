...
impl TaskHandler for InjectBearerHandler {
    fn execute(&self, _resource: &str, _state_name: &str, ctx: &Value) -> Result<Value> {
        ...
        let header_name = ctx.get("headerName").and_then(|v| v.as_str()).unwrap_or("Authorization");
        let scheme = ctx.get("scheme").and_then(|v| v.as_str()).unwrap_or("Bearer");
        let value =
            if token.is_empty() { String::from(scheme) } else { format!("{} {}", scheme, token) };
        Ok(json!({
...
impl TaskHandler for InjectApiKeyHandler {
    fn execute(&self, _resource: &str, _state_name: &str, ctx: &Value) -> Result<Value> {
        ...
        let location = ctx.get("location").and_then(|v| v.as_str()).unwrap_or("header");
        let name = ctx.get("name").and_then(|v| v.as_str()).unwrap_or("X-API-Key");
        let prefix = ctx.get("prefix").and_then(|v| v.as_str()).unwrap_or("");
        let val = if prefix.is_empty() { key.to_string() } else { format!("{} {}", prefix, key) };
        match location {
