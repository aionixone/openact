...
use openact_runtime::{
    registry_from_records, execute_action, ExecutionOptions, default_feature_flags,
    records_from_inline_config
};
...
pub async fn execute(
    ...
    let connections = config_value.get("connections")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().cloned().collect::<Vec<_>>());

    let actions = config_value.get("actions")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().cloned().collect::<Vec<_>>());
    ...
    info!("Parsed {} connections and {} actions", 
        connection_records.len(), action_records.len());
    ...
    let action_trn = action_records.iter()
        ...
        .ok_or_else(|| {
            CliError::ActionNotFound(format!(
                "Action '{}' not found in configuration",
                action_name
            ))
        })?
        .trn.clone();
