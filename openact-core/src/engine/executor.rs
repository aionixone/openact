...
// AuthFlow 连接存储（内存缓存）
static AUTHFLOW_STORE: OnceCell<Arc<dyn ConnectionStore>> = OnceCell::new();

async fn get_authflow_store() -> Arc<dyn ConnectionStore> {
...
pub struct TaskExecutor {
    ...
    authflow_handler: DefaultRouter,
}
...
impl std::fmt::Debug for TaskExecutor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TaskExecutor")
            ...
            .field("authflow_handler", &"DefaultRouter")
            .finish()
    }
}
            ...
            match resp {
                Ok(r) => {
                    ...
                    if status.as_u16() == 401 
                        ...
                        let store = get_authflow_store().await;
...
mod tests {
    ...
    async fn it_persists_oauth_token_across_restarts() {
        ...
        let got2 = get_authflow_store().await.get(&connection_key).await.unwrap();
        assert!(got2.is_some());
