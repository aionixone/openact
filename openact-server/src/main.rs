use axum::{routing::{get, post}, Router, Json, http::StatusCode, extract::{Path, State, Query}, response::IntoResponse};
...
async fn main() {
    ...
    let db = CoreDatabase::connect(&database_url).await.expect("db connect");
    let state = Arc::new(AppState { db, database_url: database_url.clone(), sessions: Mutex::new(HashMap::new()) });
    ...
    let app = Router::new()
        ...
        .route("/api/v1/actions/:trn", get(inspect_action).put(update_action).delete(delete_action))
        .route("/api/v1/actions/:trn/export", get(export_action))
        .route("/api/v1/bindings", post(bind_binding).get(list_bindings).delete(delete_binding))
        .route("/api/v1/run", post(run_action))
        .route("/api/v1/executions/:exec_trn", get(get_execution))
        .route("/api/v1/sessions", get(list_sessions).delete(clean_sessions))
...
struct AppState { db: CoreDatabase, database_url: String, sessions: Mutex<HashMap<String, serde_json::Value>> }

#[derive(Serialize, ToSchema)]
struct HealthResp { ok: bool }
...
async fn status(State(state): State<Arc<AppState>>) -> (StatusCode, Json<StatusResp>) {
    let stats = state.db.stats().await.unwrap_or(openact_core::database::CoreStats { bindings: 0, actions: 0, auth_connections: 0 });
    ...
    (StatusCode::OK, Json(StatusResp { bindings: stats.bindings, actions: stats.actions, auth_connections: stats.auth_connections, encryption: enc }))
}

#[derive(Deserialize, ToSchema)]
struct DoctorReq {
    #[serde(default)] dsl: Option<String>,
    #[serde(default = "default_port_start", rename = "portStart")] port_start: u16,
    #[serde(default = "default_port_end", rename = "portEnd")] port_end: u16,
}
fn default_port_start() -> u16 { 8080 }
fn default_port_end() -> u16 { 8099 }

#[derive(Serialize, ToSchema)]
struct DoctorResp { db_url: String, master_key_set: bool, db_connectivity: bool, free_ports_sample: Vec<u16>, dsl_check: Option<String>, suggestions: Vec<String> }
...
async fn doctor(State(state): State<Arc<AppState>>, Json(req): Json<DoctorReq>) -> (StatusCode, Json<DoctorResp>) {
    // DB URL and master key
    let db_url = std::env::var("OPENACT_DATABASE_URL").or_else(|_| std::env::var("AUTHFLOW_SQLITE_URL")).unwrap_or_else(|_| state.database_url.clone());
    let master_set = std::env::var("OPENACT_MASTER_KEY").is_ok() || std::env::var("AUTHFLOW_MASTER_KEY").is_ok();
    let mut suggestions: Vec<String> = Vec::new();
    if db_url.is_empty() { if let Ok(pwd) = std::env::current_dir() { suggestions.push(format!("export OPENACT_DATABASE_URL=sqlite:{}/manifest/data/openact.db", pwd.display())); } }
    if !master_set { suggestions.push("export OPENACT_MASTER_KEY=your-32-bytes-key".to_string()); }

    // DB connectivity
    let db_ok = state.db.health_check().await.is_ok();
    if !db_ok { suggestions.push("Check database URL and file permissions".to_string()); }

    // Ports sample
    let mut free_ports = Vec::new();
    for p in req.port_start..=req.port_end { if std::net::TcpListener::bind(("127.0.0.1", p)).is_ok() { free_ports.push(p); if free_ports.len() >= 3 { break; } } }
    if free_ports.is_empty() { suggestions.push("Choose different --portStart/--portEnd or pass --redirect to auth endpoints".to_string()); }

    // DSL secrets check
    let mut dsl_check: Option<String> = None;
    if let Some(path) = req.dsl {
        let orch = AuthOrchestrator::new(state.db.pool().clone());
        let res = orch.begin_oauth_from_config("doctor", std::path::Path::new(&path), Some("OAuth"), Some("http://localhost:8080/oauth/callback"), Some("user:email")).await;
        match res { Ok((url, _)) => { dsl_check = Some(format!("OK: {}", url)); }, Err(e) => { dsl_check = Some(format!("{}", e)); if !e.to_string().contains("authorize_url") { suggestions.push("Ensure OPENACT_SECRETS_FILE or environment secrets are set".to_string()); } } }
    }

    (StatusCode::OK, Json(DoctorResp { db_url, master_key_set: master_set, db_connectivity: db_ok, free_ports_sample: free_ports, dsl_check, suggestions }))
}
...
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct ErrorResp { error: ErrorBody }
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct DeleteResp { deleted: bool }
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct UnboundResp { unbound: bool }

// ===== OAuth Begin/Complete =====

#[derive(Deserialize, ToSchema)]
struct OauthBeginReq { tenant: String, dsl: String, #[serde(default)] flow: Option<String>, #[serde(default, rename = "redirectUri")] redirect_uri: Option<String>, #[serde(default)] scope: Option<String> }
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct OauthBeginResp { authorize_url: String, session_id: String, redirect_uri: Option<String> }
...
async fn oauth_begin(State(state): State<Arc<AppState>>, Json(req): Json<OauthBeginReq>) -> (StatusCode, Json<serde_json::Value>) {
    let orch = AuthOrchestrator::new(state.db.pool().clone());
    let path = std::path::Path::new(&req.dsl);
    let (auth_url, pending) = match orch.begin_oauth_from_config(&req.tenant, path, req.flow.as_deref(), req.redirect_uri.as_deref(), req.scope.as_deref()).await {
        Ok(v) => v,
        Err(e) => { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": e.to_string()}}))); }
    };
    let sid = Uuid::new_v4().to_string();
    let mut sess = state.sessions.lock().await;
    // try extract state from authorize_url ("...state=xxxxx&...")
    let state_q = auth_url
        .split('?')
        .nth(1)
        .and_then(|q| q.split('&').find(|kv| kv.starts_with("state=")).map(|kv| kv.trim_start_matches("state=").to_string()));
...
async fn oauth_callback(State(state): State<Arc<AppState>>, Query(params): Query<HashMap<String, String>>) -> (StatusCode, Json<serde_json::Value>) {
    let code_opt = params.get("code").cloned();
    let state_opt = params.get("state").cloned();
    // find session by state if provided, else fail
    if state_opt.is_none() {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": "missing state"}})));
    }
    let state_key = format!("state:{}", state_opt.clone().unwrap());
    let mut sessions = state.sessions.lock().await;
    let Some(v) = sessions.get(&state_key).cloned() else {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": "session not found for state"}})));
    };
    ...
    let _tenant = v.get("tenant").and_then(|x| x.as_str()).unwrap_or("default").to_string();
    let dsl_path = v.get("dsl").and_then(|x| x.as_str()).unwrap_or("").to_string();
    let flow = v.get("flow_name").and_then(|x| x.as_str()).unwrap_or("OAuth").to_string();
    let next_state = v.get("next_state").and_then(|x| x.as_str()).unwrap_or("").to_string();
    ...
    if let Some(code) = code_opt.clone() {
        if let serde_json::Value::Object(ref mut o) = context { o.insert("code".into(), serde_json::Value::String(code)); }
    }
    if let Some(st) = state_opt.clone() {
        if let serde_json::Value::Object(ref mut o) = context { o.insert("state".into(), serde_json::Value::String(st)); }
    }

    let orch = AuthOrchestrator::new(state.db.pool().clone());
    let path = std::path::Path::new(&dsl_path);
    match orch.resume_with_context(path, &flow, &next_state, context).await {
...
#[derive(Deserialize, ToSchema)]
struct OauthCompleteReq { tenant: String, dsl: String, session_id: String, #[serde(default)] callback_url: Option<String>, #[serde(default)] code: Option<String>, #[serde(default)] state: Option<String> }
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct OauthCompleteResp { auth_trn: String }
...
async fn oauth_complete(State(state): State<Arc<AppState>>, Json(req): Json<OauthCompleteReq>) -> (StatusCode, Json<serde_json::Value>) {
    let orch = AuthOrchestrator::new(state.db.pool().clone());
    let path = std::path::Path::new(&req.dsl);
    // load session
    let mut sessions = state.sessions.lock().await;
    let Some(v) = sessions.remove(&req.session_id) else { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": "invalid session"}}))); };
    let flow = v.get("flow_name").and_then(|x| x.as_str()).unwrap_or("OAuth").to_string();
    let next_state = v.get("next_state").and_then(|x| x.as_str()).unwrap_or("").to_string();
    let mut context = v.get("context").cloned().unwrap_or(serde_json::json!({}));
    drop(sessions);
    ...
    if let Some(code) = req.code.clone() { if let serde_json::Value::Object(ref mut o) = context { o.insert("code".into(), serde_json::Value::String(code)); } }
    if let Some(state_s) = req.state.clone() { if let serde_json::Value::Object(ref mut o) = context { o.insert("state".into(), serde_json::Value::String(state_s)); } }
    ...
    if let Some(cb) = req.callback_url.clone() {
        // Reconstruct pending
        let (_auth_url, pend) = match orch.begin_oauth_from_config(&req.tenant, path, Some(&flow), None, None).await { Ok(v) => v, Err(e) => { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": e.to_string()}}))); } };
...
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct AuthRefreshResp { auth_trn: String, refreshed: bool }
...
async fn auth_refresh(State(state): State<Arc<AppState>>, Path(trn): Path<String>, Json(req): Json<AuthRefreshReq>) -> (StatusCode, Json<serde_json::Value>) {
    match AuthManager::from_database_url(state.database_url.clone()).await {
        Ok(am) => {
            // Get current connection
            let conn = match am.get(&trn).await {
                Ok(Some(c)) => c,
                Ok(None) => return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": {"code": "NotFound", "message": "auth connection not found"}}))),
                Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}}))),
            };

            // Check if refresh_token exists
            let refresh_token = match &conn.refresh_token {
                Some(rt) => rt,
                None => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": "no refresh token available"}}))),
            };
            ...
            match handler.execute("oauth2.refresh_token", "refresh", &refresh_req) {
                Ok(result) => {
                    // Extract new tokens
                    let access_token = result.get("access_token").and_then(|v| v.as_str()).unwrap_or("");
                    ...
                    // Update connection
                    match am.refresh_connection(&trn, access_token, new_refresh_token, expires_in).await {
                        Ok(_) => (StatusCode::OK, Json(serde_json::json!({"auth_trn": trn, "refreshed": true}))),
                        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": format!("failed to update connection: {}", e)}})))
                    }
                }
                Err(e) => (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": format!("token refresh failed: {}", e)}})))
            }
        }
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}})))
    }
}

// ===== PAT Create =====
#[derive(Deserialize, ToSchema)]
struct PatReq { tenant: String, provider: String, user_id: String, token: String }
#[allow(dead_code)]
#[derive(Serialize, ToSchema)]
struct PatResp { auth_trn: String }
...
async fn auth_pat(State(state): State<Arc<AppState>>, Json(req): Json<PatReq>) -> (StatusCode, Json<serde_json::Value>) {
    match AuthManager::from_database_url(state.database_url.clone()).await {
        Ok(am) => match am.create_pat_connection(&req.tenant, &req.provider, &req.user_id, &req.token).await {
            Ok(trn) => (StatusCode::OK, Json(serde_json::json!({"auth_trn": trn}))),
            Err(e) => (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": e.to_string()}})))
        },
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}})))
...
async fn list_sessions(State(state): State<Arc<AppState>>) -> (StatusCode, Json<serde_json::Value>) {
    let sessions = state.sessions.lock().await;
    let session_list: Vec<serde_json::Value> = sessions.iter().map(|(id, data)| {
        serde_json::json!({
            "session_id": id,
    ...
    (StatusCode::OK, Json(serde_json::json!({"sessions": session_list, "count": session_list.len()})))
}

#[utoipa::path(delete, path = "/api/v1/sessions", responses((status = 200, description = "OK")))]
async fn clean_sessions(State(state): State<Arc<AppState>>) -> (StatusCode, Json<serde_json::Value>) {
    let mut sessions = state.sessions.lock().await;
...
async fn get_session(State(state): State<Arc<AppState>>, Path(id): Path<String>) -> (StatusCode, Json<serde_json::Value>) {
    let sessions = state.sessions.lock().await;
    match sessions.get(&id) {
        Some(data) => (StatusCode::OK, Json(data.clone())),
        None => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": {"code": "NotFound", "message": "session not found"}})))
    }
}
...
async fn delete_session(State(state): State<Arc<AppState>>, Path(id): Path<String>) -> (StatusCode, Json<serde_json::Value>) {
    let mut sessions = state.sessions.lock().await;
...
// ===== Run & Executions =====
#[derive(Deserialize, ToSchema)]
struct RunReq { tenant: String, action_trn: String, #[serde(default)] exec_trn: Option<String>, #[serde(default)] output: Option<String>, #[serde(default)] dry_run: Option<bool>, #[serde(default)] trace: Option<bool> }
...
async fn get_execution(State(state): State<Arc<AppState>>, Path(exec_trn): Path<String>) -> (StatusCode, Json<serde_json::Value>) {
    let erepo = ExecutionRepository::new(state.db.pool().clone());
...
#[derive(Deserialize, ToSchema)]
struct UnbindReq { tenant: String, auth_trn: String, action_trn: String }
...
#[derive(Deserialize, ToSchema)]
struct BindReq { tenant: String, auth_trn: String, action_trn: String }

#[utoipa::path(post, path = "/api/v1/bindings", request_body = BindReq, responses((status = 200, description = "OK")))]
async fn bind_binding(State(state): State<Arc<AppState>>, Json(req): Json<BindReq>) -> (StatusCode, Json<serde_json::Value>) {
    let bm = BindingManager::new(state.db.pool().clone());
    match bm.bind(&req.tenant, &req.auth_trn, &req.action_trn, Some("api")).await {
        Ok(b) => (StatusCode::OK, Json(serde_json::json!({"tenant": b.tenant, "auth_trn": b.auth_trn, "action_trn": b.action_trn}))),
        Err(e) => (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": {"code": "BadRequest", "message": e.to_string()}})))
    }
}

#[derive(Deserialize, ToSchema)]
struct ListBindingsQuery { #[serde(default)] tenant: Option<String>, #[serde(default)] auth_trn: Option<String>, #[serde(default)] action_trn: Option<String>, #[serde(default)] verbose: Option<bool> }

#[utoipa::path(get, path = "/api/v1/bindings", responses((status = 200, description = "OK")))]
async fn list_bindings(State(state): State<Arc<AppState>>, Query(q): Query<ListBindingsQuery>) -> (StatusCode, Json<serde_json::Value>) {
...
// ===== Auth list/inspect =====
#[derive(Deserialize, ToSchema)]
struct AuthListQuery { #[serde(default)] _tenant: Option<String>, #[serde(default)] _provider: Option<String> }

#[utoipa::path(get, path = "/api/v1/auth", responses((status = 200, description = "OK")))]
async fn list_auth(State(state): State<Arc<AppState>>, Query(_q): Query<AuthListQuery>) -> (StatusCode, Json<serde_json::Value>) {
    match AuthManager::from_database_url(state.database_url.clone()).await {
        Ok(am) => match am.list().await { Ok(v) => (StatusCode::OK, Json(serde_json::json!({"items": v}))), Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}}))) },
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}})))
    }
}

#[utoipa::path(get, path = "/api/v1/auth/{trn}", responses((status = 200, description = "OK")))]
async fn get_auth(State(state): State<Arc<AppState>>, Path(trn): Path<String>) -> (StatusCode, Json<serde_json::Value>) {
    match AuthManager::from_database_url(state.database_url.clone()).await {
        Ok(am) => match am.get(&trn).await { Ok(Some(conn)) => (StatusCode::OK, Json(serde_json::json!({"trn": conn.trn.to_trn_string().unwrap_or_default(), "token_type": conn.token_type, "scope": conn.scope, "extra": conn.extra}))), Ok(None) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": {"code": "NotFound", "message": "auth not found"}}))), Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}}))) },
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}})))
    }
}
...
#[derive(Deserialize, ToSchema)]
struct RegisterActionReq { tenant: String, provider: String, name: String, trn: String, yaml: String }

#[utoipa::path(get, path = "/api/v1/actions", responses((status = 200, description = "OK")))]
async fn list_actions(State(state): State<Arc<AppState>>, Query(q): Query<HashMap<String, String>>) -> (StatusCode, Json<serde_json::Value>) {
    let tenant = q.get("tenant").cloned().unwrap_or_else(|| "default".to_string());
    let registry = ActionRegistry::new(state.db.pool().clone());
    match registry.list_by_tenant(&tenant).await {
        Ok(items) => (StatusCode::OK, Json(serde_json::json!({"items": items.into_iter().map(|a| { serde_json::json!({"trn": a.trn, "tenant": a.tenant, "provider": a.provider, "name": a.name, "is_active": a.is_active}) }).collect::<Vec<_>>() })) ),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": {"code": "Internal", "message": e.to_string()}})))
    }
}

#[utoipa::path(get, path = "/api/v1/actions/{trn}", responses((status = 200, description = "OK")))]
async fn inspect_action(State(state): State<Arc<AppState>>, Path(trn): Path<String>) -> (StatusCode, Json<serde_json::Value>) {
    let registry = ActionRegistry::new(state.db.pool().clone());
    match registry.get_by_trn(&trn).await {
        Ok(a) => (StatusCode::OK, Json(serde_json::json!({"trn": a.trn, "tenant": a.tenant, "provider": a.provider, "name": a.name, "is_active": a.is_active}))),
        Err(_) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": {"code": "NotFound", "message": "action not found"}})))
    }
}
...
async fn register_action(State(state): State<Arc<AppState>>, Json(req): Json<RegisterActionReq>) -> (StatusCode, Json<serde_json::Value>) {
    let registry = ActionRegistry::new(state.db.pool().clone());
    // write yaml to temp file to reuse registry API
    let mut f = NamedTempFile::new().unwrap();
    let _ = f.write_all(req.yaml.as_bytes());
    match registry.register_from_yaml(&req.tenant, &req.provider, &req.name, &req.trn, f.path()).await {
...
#[derive(Deserialize, ToSchema)]
struct UpdateActionReq { yaml: String }
...
async fn update_action(State(state): State<Arc<AppState>>, Path(trn): Path<String>, Json(req): Json<UpdateActionReq>) -> (StatusCode, Json<serde_json::Value>) {
    let registry = ActionRegistry::new(state.db.pool().clone());
    let mut f = NamedTempFile::new().unwrap();
    let _ = f.write_all(req.yaml.as_bytes());
...
async fn export_action(State(state): State<Arc<AppState>>, Path(trn): Path<String>) -> impl IntoResponse {
    let registry = ActionRegistry::new(state.db.pool().clone());
    match registry.export_spec_by_trn(&trn).await {
        Ok(spec) => (StatusCode::OK, [(CONTENT_TYPE, "text/yaml")], spec).into_response(),
        Err(_) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": {"code": "NotFound", "message": "action not found"}}))).into_response()
    }
}